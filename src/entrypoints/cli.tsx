...
// Disable experimental API betas by default for third-party provider compatibility.
// Tool search (defer_loading), global cache scope, and context management
// require internal Anthropic API support not available to external accounts → 500.
// Users can opt-in by setting CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=false explicitly.
...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.DISPLAY_VERSION ?? MACRO.VERSION} (Snowcode)`);
    return;
  }
  ...
  if (process.env.SNOWCODE_DISABLE_EARLY_INPUT !== '1') {
    const {
