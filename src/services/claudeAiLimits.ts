...
import { shouldShowClaudeAiLimitsForModel } from './claudeAiLimitVisibility.js'
...
function createDefaultClaudeAiLimits(): ClaudeAILimits {
  return {
    status: 'allowed',
...
function shouldExposeClaudeAiLimits(): boolean {
  return getAPIProvider() === 'firstParty' || shouldProcessMockLimits()
}

// Exported for testing only
export let currentLimits: ClaudeAILimits = createDefaultClaudeAiLimits()
...
export function getRawUtilization(model?: string): RawUtilization {
  if (!shouldShowClaudeAiLimitsForModel(model)) {
    return {}
  }
  return rawUtilization
}

export function getCurrentClaudeAiLimits(model?: string): ClaudeAILimits {
  if (!shouldShowClaudeAiLimitsForModel(model)) {
    return createDefaultClaudeAiLimits()
  }
  return currentLimits
}

function clearClaudeAiLimits(): void {
  rawUtilization = {}
  const defaultLimits = createDefaultClaudeAiLimits()
