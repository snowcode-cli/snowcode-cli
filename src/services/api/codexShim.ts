...
function enforceStrictSchema(schema: unknown): Record<string, unknown> {
  ...
  if (
    ...
    record.additionalProperties = enforceStrictSchema(record.additionalProperties)
  }
...
export async function performCodexRequest(options: {
  ...
  if (!response.ok) {
    ...
    appendHttpLog(
      `${options.request.requestedModel} error`,
      `POST ${options.request.baseUrl}/responses\n${Object.entries(headers)
  ...
  appendHttpLog(
    `${options.request.requestedModel} success`,
    `POST ${options.request.baseUrl}/responses\n${Object.entries(headers)
      .map(([key, value]) => `${key}: ${value}`)
      .join('\n')}\n\n${JSON.stringify(body)}`,
