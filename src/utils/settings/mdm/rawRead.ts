...
function execFilePromise(
  ...
  return new Promise(resolve => {
    try {
      ...
      // Treat subprocess spawn failures as "no managed settings" rather than a
      // fatal startup error. On Windows, `reg query` can fail to spawn under
      // some parent process setups even though the CLI should still run.
