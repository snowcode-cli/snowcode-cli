 ...
 *
 * 1. Managed memory (eg. /etc/claude-code/SNOW.md) - Global instructions for all users
 * 2. User memory (~/.claude/SNOW.md) - Private global instructions for all projects
 * 3. Project memory (SNOW.md, .claude/SNOW.md, and .claude/rules/*.md in project roots) - Instructions checked into the codebase
 * 4. Local memory (SNOW.local.md in project roots) - Private project-specific instructions
 *
 ...
 * - SNOW.md, .claude/SNOW.md, and all .md files in .claude/rules/ are checked in each directory for Project memory
 *
...
/**
 * Checks whether a SNOW.md file path is excluded by the claudeMdExcludes setting.
...
function isClaudeMdExcluded(filePath: string, type: MemoryType): boolean {
  ...
  // the user writes "/tmp/project/SNOW.md" in their exclude, but the system
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // checked-in files like SNOW.md and .claude/rules/*.md, so the same
    // content gets loaded twice. Skip Project-type (checked-in) files from
    // directories above the worktree but within the main repo — the worktree
    // already has its own checkout. SNOW.local.md is gitignored so it only
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading SNOW.md (Project) - only if projectSettings is enabled
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        const projectPath = join(dir, 'SNOW.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
            'Project',
        ...
        // Try reading .claude/SNOW.md (Project)
        const dotClaudePath = join(dir, '.claude', 'SNOW.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
            'Project',
      ...
      // Try reading SNOW.local.md (Local) - only if localSettings is enabled
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = join(dir, 'SNOW.local.md')
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
    ...
    // Process SNOW.md from additional directories (--add-dir) if env var is enabled
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        // Try reading SNOW.md from the additional directory
        const projectPath = join(dir, 'SNOW.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
        ...
        // Try reading .claude/SNOW.md from the additional directory
        const dotClaudePath = join(dir, '.claude', 'SNOW.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
    ...
    // memory system, not "instructions" in the SNOW.md/rules sense.
 ...
 * Loads SNOW.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process project memory files (SNOW.md and .claude/SNOW.md)
  ...
  // Process local memory file (SNOW.local.md)
  if (isSettingSourceEnabled('localSettings')) {
    const localPath = join(dir, 'SNOW.local.md')
    result.push(
...
/**
 * Check if a file path is a memory file (SNOW.md, SNOW.local.md, or .claude/rules/*.md)
 */
export function isMemoryFilePath(filePath: string): boolean {
  const name = basename(filePath)

  // SNOW.md or SNOW.local.md anywhere
  if (name === 'SNOW.md' || name === 'SNOW.local.md') {
    return true
  }
