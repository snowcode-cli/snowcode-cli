...
export type ProfileEnv = {
  ...
  CLAUDE_CODE_COMPACT_MODEL?: string
...
function applyCompactOverrides(
  env: NodeJS.ProcessEnv,
  processEnv: NodeJS.ProcessEnv,
  persistedEnv: ProfileEnv,
): void {
  const compactModel =
    processEnv.CLAUDE_CODE_COMPACT_MODEL ||
    persistedEnv.CLAUDE_CODE_COMPACT_MODEL
  if (compactModel) {
    env.CLAUDE_CODE_COMPACT_MODEL = compactModel
  } else {
    delete env.CLAUDE_CODE_COMPACT_MODEL
  }

  const autoCompactWindow =
    processEnv.CLAUDE_CODE_AUTO_COMPACT_WINDOW ||
    persistedEnv.CLAUDE_CODE_AUTO_COMPACT_WINDOW
...
export async function buildLaunchEnv(options: {
  ...
  if (options.profile === 'gemini') {
    ...
    applyCompactOverrides(env, processEnv, persistedEnv)
    return env
  }
