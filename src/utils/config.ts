...
export type GlobalConfig = {
  ...
  /** headroom automatic input compression — true = enabled */
...
export function getMemoryPath(memoryType: MemoryType): string {
  ...
  switch (memoryType) {
    case 'User':
      return join(getClaudeConfigHomeDir(), 'SNOW.md')
    case 'Local':
      return join(cwd, 'SNOW.local.md')
    case 'Project':
      return join(cwd, 'SNOW.md')
    case 'Managed':
      return join(getManagedFilePath(), 'SNOW.md')
    case 'AutoMem':
