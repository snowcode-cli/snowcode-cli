...
const missingCodexAuthPath = join(tmpdir(), 'snowcode-missing-codex-auth.json')
...
test('launch env carries persisted compact overrides', async () => {
  const env = await buildLaunchEnv({
    profile: 'openai',
    persisted: profile('openai', {
      OPENAI_BASE_URL: 'https://api.openai.com/v1',
      OPENAI_MODEL: 'gpt-4o',
      OPENAI_API_KEY: 'sk-persisted',
      CLAUDE_CODE_COMPACT_MODEL: 'gpt-4o-mini',
  ...
  assert.equal(env.CLAUDE_CODE_COMPACT_MODEL, 'gpt-4o-mini')
  assert.equal(env.CLAUDE_CODE_AUTO_COMPACT_WINDOW, '120000')
})

test('shell compact overrides beat persisted profile values', async () => {
  const env = await buildLaunchEnv({
    profile: 'openai',
    persisted: profile('openai', {
      OPENAI_BASE_URL: 'https://api.openai.com/v1',
    ...
    processEnv: {
      OPENAI_API_KEY: 'sk-live',
      CLAUDE_CODE_COMPACT_MODEL: 'o4-mini',
      CLAUDE_CODE_AUTO_COMPACT_WINDOW: '90000',
    },
  })

  assert.equal(env.CLAUDE_CODE_COMPACT_MODEL, 'o4-mini')
