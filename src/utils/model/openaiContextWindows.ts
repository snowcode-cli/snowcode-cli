...
import { getProviderScopedBaseModel } from './providerMetadata.js'
...
export function getOpenAIContextWindow(model: string): number | undefined {
  return lookupByModel(OPENAI_CONTEXT_WINDOWS, getProviderScopedBaseModel(model))
}
...
export function getOpenAIMaxOutputTokens(model: string): number | undefined {
  return lookupByModel(
    ...
    getProviderScopedBaseModel(model),
  )
}
