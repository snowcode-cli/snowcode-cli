...
import { getInferenceControlDescriptor } from '../../utils/reasoningControl.js';
...
import { getInferenceControlNotificationText } from '../EffortIndicator.js';
...
function PromptInput({
  ...
  const mainLoopInferenceDescriptor = useMemo(() => getInferenceControlDescriptor(mainLoopModel), [mainLoopModel]);

  // Handler for chat:thinkingToggle - only available on models with thinking on/off
  const handleThinkingToggle = useCallback(() => {
    if (mainLoopInferenceDescriptor.kind !== 'toggle') {
      addNotification({
        key: 'thinking-toggle-unavailable',
        text: mainLoopInferenceDescriptor.kind === 'levels' ? `Use /${mainLoopInferenceDescriptor.commandName} or the model picker to adjust ${mainLoopInferenceDescriptor.label} for this model` : 'Inference controls are not available for this model',
        priority: 'high',
        timeoutMs: 4000
      });
      if (helpOpen) {
  ...
  }, [addNotification, helpOpen, mainLoopInferenceDescriptor]);
  ...
  const effortNotificationText = briefOwnsGap ? undefined : getInferenceControlNotificationText(effortValue, thinkingEnabled, mainLoopModel);
  useEffect(() => {
  ...
  const handleModelSelect = useCallback((model: string | null, controlSummary: string | undefined) => {
  ...
  const thinkingToggleElement = useMemo(() => {
    if (!showThinkingToggle || mainLoopInferenceDescriptor.kind !== 'toggle') return null;
    return <Box flexDirection="column" marginTop={1}>
  ...
  }, [showThinkingToggle, mainLoopInferenceDescriptor, thinkingEnabled, handleThinkingSelect, handleThinkingCancel, messages.length]);
