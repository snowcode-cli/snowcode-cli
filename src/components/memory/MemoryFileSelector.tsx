...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getClaudeConfigHomeDir(), "SNOW.md");
  const projectMemoryPath = join(getOriginalCwd(), "SNOW.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.claude/SNOW.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./SNOW.md`;
      } else {
        if (file.parent) {
