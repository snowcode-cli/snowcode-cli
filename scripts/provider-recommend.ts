...
function parseOptions(argv: string[]): CliOptions {
  const options: CliOptions = {
    apply: false,
    benchmark: false,
    goal: normalizeRecommendationGoal(process.env.SNOWCODE_PROFILE_GOAL),
    json: false,
    provider: 'auto',
    baseUrl: null,
  }
...
function printHumanSummary(payload: {
  ...
  if (payload.applied) {
    console.log('\nSaved .snowcode-profile.json with the recommended profile.')
...
async function maybeApplyProfile(
  ...
  writeFileSync(
    resolve(process.cwd(), '.snowcode-profile.json'),
