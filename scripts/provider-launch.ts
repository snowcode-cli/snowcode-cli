...
function parseLaunchOptions(argv: string[]): LaunchOptions {
  ...
  let fast = false
  let goal = normalizeRecommendationGoal(process.env.SNOWCODE_PROFILE_GOAL)
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if ((lower === 'auto' || lower === 'openai' || lower === 'ollama' || lower === 'codex' || lower === 'gemini' || lower === 'vertex') && requestedProfile === 'auto') {
...
function loadPersistedProfile(): ProfileFile | null {
  const path = resolve(process.cwd(), '.snowcode-profile.json')
  if (!existsSync(path)) return null
  try {
    const parsed = JSON.parse(readFileSync(path, 'utf8')) as ProfileFile
    if (parsed.profile === 'openai' || parsed.profile === 'ollama' || parsed.profile === 'codex' || parsed.profile === 'gemini' || parsed.profile === 'vertex') {
      return parsed
    }
    return null
  } catch {
